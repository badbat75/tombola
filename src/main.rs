...
fn print_board(board: &[u8], extracted: u8) {
    let mut sorted_board = board.to_owned();
    sorted_board.sort();
    let mut prev_num = 0;
    for &curr_num in &sorted_board {
...
fn print_last_numbers(board: &[u8], n: usize)  -> Vec<u8> {
    if board.len() <= 1 {
        return Vec::new();
    }
    ...
    let mut result: Vec<u8> = board[start_index..end_index].to_vec();
    result.reverse();
    result
}

fn score_check(board: &[u8], extracted: u8, scorecard: &mut u8) {
    let numbers_per_row = (BOARDCONFIG.cols_per_card * BOARDCONFIG.cards_per_row) as i8;
    ...
    for &num in board {
        if num == extracted {
            continue; // Skip the extracted number itself
        }
    ...
    // Check for complete cards (15 numbers)
    ...
    for card_index in 0..total_cards {
        ...
        for row in 0..BOARDCONFIG.rows_per_card as i8 {
            for col in 0..BOARDCONFIG.cols_per_card as i8 {
                ...
                if board.contains(&(number as u8)) {
    ...
    // Update scorecard for line scoring if no complete card found
    if *scorecard != numbers_per_card as u8 && same_row_card_count + 1 > *scorecard { 
        *scorecard = same_row_card_count + 1;
    }
...
fn main() {
    ...
    while ! pouch.is_empty() {
        ...
        score_check(&board, extracted, &mut scorecard);
